...
class OrderRepository extends AbstractRepository
{
    ...
    // === 追加関数
    public function getOrderCountByStatus($resellerId = null, $companyId = null)
    {
        $qb = $this->createQueryBuilder('o')
            ->select('IDENTITY(o.OrderStatus) as status_id, COUNT(o.id) as cnt')
            ->groupBy('o.OrderStatus');

        // ★ ここで絞り込み
        if ($resellerId !== null) {
            $qb->andWhere('o.reseller_id = :resellerId')
               ->setParameter('resellerId', $resellerId);
        }

        if ($companyId !== null) {
            $qb->andWhere('o.company_id = :companyId')
